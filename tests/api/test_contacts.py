...
def data_to_delete(scope="class"):
    return {"firstName": "Jeanna", "lastName": "Smith", "email": "jsmith@fake.com"}
...
def test_get_contact_list(headers_with_auth, login_data):
    ...
    try:
        ...
        logger.error(
            f"Failed to parse contact list response as JSON. Response: {response.text}"
        )
        raise e
...
def test_add_contact(headers_with_auth, login_data, contact_data):
    ...
    ), f"Failed to add contact. Status code: {response.status_code}, Response: {response.text}"
    ...
    contact = next(
        (
            contact
            for contact in contact_list_response.json()
    ...
    assert (
        contact
    ), f"Contact with email {payload['email']} not found in the contact list."
    contact_id = contact["_id"]

    # Cleanup: Delete the created contact
    delete_response = requests.delete(
        f"{CONTACT_LIST_URL}/{contact_id}", headers=headers_with_auth
    )
    logger.debug(
        f"Delete Contact Response: {delete_response.status_code}, {delete_response.text}"
    )
    assert (
        delete_response.status_code == 200
    ), "Cleanup failed: Unable to delete the contact."
...
def test_update_contact(headers_with_auth, login_data, contact_data):
    ...
    updated_contact_data = {"firstName": "Albert", "phone": "8006666666"}
    ...
    assert (
        contact
    ), f"Contact with email {contact_data['email']} not found in the contact list."
    contact_id = contact["_id"]

    # Send the PATCH request to update the contact
    update_response = requests.patch(
        f"{CONTACT_LIST_URL}/{contact_id}",
        headers=headers_with_auth,
        json=updated_contact_data,
    )
    logger.debug(
        f"Update Contact Response: {update_response.status_code}, {update_response.text}"
    )

    # Assert that the contact was successfully updated
    assert (
        update_response.status_code == 200
    ), f"Failed to update contact. Status code: {update_response.status_code}, Response: {update_response.text}"

    # Verify that the contact was updated correctly
    updated_contact = update_response.json()
    assert (
        updated_contact["firstName"] == updated_contact_data["firstName"]
    ), f"First name was not updated."
    assert (
        updated_contact["phone"] == updated_contact_data["phone"]
    ), f"Phone number was not updated."
...
def test_delete_contact(headers_with_auth, login_data, contact_data):
    ...
    contact = next(
        (c for c in contact_list if c.get("email") == payload["email"]), None
    )
    ...
    assert (
        delete_response.status_code == 200
    ), f"Failed to delete contact. Status code: {delete_response.status_code}"

    # Verify the contact no longer exists
    contact_list_response = requests.get(CONTACT_LIST_URL, headers=headers_with_auth)
    assert (
        contact_list_response.status_code == 200
    ), "Failed to retrieve contact list after deletion."
    contact_list = contact_list_response.json()
    assert not any(
...
def test_delete_as_cleanup_for_ui_tests(headers_with_auth, login_data, data_to_delete):
    ...
    contact_list = contact_list_response.json()
    contact = next(
        (c for c in contact_list if c.get("email") == data_to_delete["email"]), None
    )
    if not contact:
        logger.info(
            f"Contact with email {data_to_delete['email']} not found. Skipping cleanup."
        )
