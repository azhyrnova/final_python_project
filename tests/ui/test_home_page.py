...
def wrong_login_data():
    return {"email": "fakelogin@fake.com", "password": "1234"}
...
def test_user_can_login_with_correct_credentials(page, login_data):
    ...
    assert (
        contact_page.is_logout_button_visible()
    ), "Logout button is not visible, so login was not successful"
    assert (
        contact_page.is_add_new_contact_button_visible()
    ), "Add new contact button is not visible, so login was not successful"
...
def test_user_cannot_login_with_wrong_credentials(page, wrong_login_data):
    ...
    home_page.fill_in_login_data(
        email=wrong_login_data["email"], password=wrong_login_data["password"]
    )
