...
def new_contact_data():
    return {"first_name": "Jeanna", "last_name": "Smith", "email": "jsmith@fake.com"}
...
def test_user_can_add_a_new_contact(page, login_data, new_contact_data):
    ...
    contact_data_as_string = " ".join(
        [new_contact_data["first_name"], new_contact_data["last_name"]]
    )
    ...
    assert (
        locator_element.is_visible()
    ), f"New contact '{contact_data_as_string}' not found in the contact list."

    logger.info(
        f"Contact '{contact_data_as_string}' successfully added to the contact list."
    )
